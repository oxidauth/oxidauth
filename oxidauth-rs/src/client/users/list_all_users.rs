...
pub struct ClientMock {
    pub list_all_users: Option<Arc<dyn Fn(ListAllUsersReq) -> Result<ListAllUsersRes, BoxedError> + Send + Sync>>,
}
...
#[cfg(feature = "mock")]
#[async_trait]
impl ListAllUsersTrait for Arc<Mutex<Option<Result<ListAllUsersRes, BoxedError>>>> {
    async fn list_all_users<T>(
        &self,
        params: T,
    ) -> Result<ListAllUsersRes, BoxedError>
