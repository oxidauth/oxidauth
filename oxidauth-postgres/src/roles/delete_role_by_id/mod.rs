...
impl DeleteRoleById for Database {
    async fn delete_role_by_id(&self, role_id: Uuid) -> Result<RoleRow, DeleteRoleByIdError> {
        let result = sqlx::query_as::<_, super::PgRole>(include_str!("./delete_role_by_id.sql"))
            .bind(role_id)
            .fetch_one(&self.pool)
            .await
            .map(Into::into)
...
mod tests {
    ...
    #[sqlx::test]
    async fn it_should_delete_a_role_by_id_successfully(pool: PgPool) {
        ...
        match db.delete_role_by_id(role_id).await {
            Ok(role) => {
